import { getBasicInfo } from "./getInfoModal";

export const getPosts = async (search, limit) => {
  const data = {
    jsonrpc: "2.0",
  ...
  try {
    ...
    if (Array.isArray(responseData.result)) {
      const markdownPosts = responseData.result.filter(
        (post) =>
          !/<div>|<center>|<sub>|<br>|<div class=|<a|<a href=""|<img|<img src=""|<sup>/.test(
            post.body
          )
      );
...
export const getTotalPosts = async (search, limit) => {
  ...
  try {
    ...
    if (Array.isArray(responseData.result)) {
      ...
      const basicInfo = await getBasicInfo(search);
